...
class CogRepLKNet(nn.Module):
    def __init__(self, in_feature=64,num_classes=2, use_kan=False, pretrained=False, save_feature=False, drop_rate=0.):
        ...
        self.UniRepLKNet = timm.create_model('CogRepLKNet', drop_path_rate=drop_rate)
...
class TrainAndEval:
    def __init__(self, data_path, data_path_r, log_path, epoch, lr, bs, save_model=False):
        ...
        self.allData_r = torch.from_numpy(S1Data_r)
    ...
    def train_and_val(self, model, model_r, class_head, nSub):
        ...
        for epoch in range(self.epoch):
            ...
            for i, ((inputs, labels), (inputs_r, labels_r)) in enumerate(tqdm(zip(self.train_loader, self.train_loader_r), desc=f"Epoch {self.epoch}/{epoch + 1}")):
                ...
                #    print(len(inputs))
                aug_data, aug_label = interaug(self.X_train, self.y_train, self.bs, 40, 2, device)
                inputs = torch.cat((inputs, aug_data))
                labels = torch.cat((labels, aug_label))

                aug_data_r, _ = finteraug(self.X_train_r, self.y_train, self.bs, 40, 2, device)
    ...
    def test_normal_rep(self, model, model_r, class_head, nSub, weight_path, rep=False):
        device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
        model.load_state_dict(torch.load(weight_path + 'model' + str(nSub) + '.pth'))
        model_r.load_state_dict(torch.load(weight_path + 'modelr' + str(nSub) + '.pth'))
        class_head.load_state_dict(torch.load(weight_path + 'class_head' + str(nSub) + '.pth'))
        if rep:
            model.UniRepLKNet.reparameterize_unireplknet()
            model_r.UniRepLKNet.reparameterize_unireplknet()
        model = model.to(device)
        model_r = model_r.to(device)
        class_head = class_head.to(device)
        model.eval()
        ...
        for (inputs, labels), (inputs_r, labels_r) in zip(self.train_loader, self.train_loader_r):
            # 将数据和标签转移到设备上
            inputs = inputs.to(device)
            labels = labels.to(device)
            inputs_r = inputs_r.to(device)
        ...
        print(nSub, " Test time: %.2f s" % (test_time))
        print(f"Accuracy of the model on the test set: {100 * correct / total:.2f}%")
        return et - st
    def cal_para(self, model, modelr, rep=False):
        if rep:
            model.UniRepLKNet.reparameterize_unireplknet()
            model_r.UniRepLKNet.reparameterize_unireplknet()
        total_params_eeg = sum(p.numel() for p in model.UniRepLKNet.parameters() if p.requires_grad)
        totao_params_fmri = sum(p.numel() for p in modelr.UniRepLKNet.parameters() if p.requires_grad)
        print("Total parameters of EEG model: ", total_params_eeg)
        print("Total parameters of fMRI model: ", totao_params_fmri)
        print("Total parameters: ", total_params_eeg + totao_params_fmri)
    
    def get_feature(self, model, model_r, class_head, nSub, weight_path):
        device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
        model.load_state_dict(torch.load(weight_path + 'model' + str(nSub) + '.pth'))
        model_r.load_state_dict(torch.load(weight_path + 'modelr' + str(nSub) + '.pth'))
        class_head.load_state_dict(torch.load(weight_path + 'class_head' + str(nSub) + '.pth'))
        ...
        model_r.eval()
        class_head.eval()
        f = model(self.allData.to(device))
        f_r = model_r(self.allData_r.to(device).float())
        eeg_feature = model.get_feature()
        frmi_feature = model_r.get_feature()
        out = class_head(f, f_r)
        eeg_w, fusion_feature = class_head.get_weight_feature()

        eeg_feature_np = eeg_feature.detach().cpu().numpy()
        frmi_feature_np = frmi_feature.detach().cpu().numpy()
        fusion_feature_np = fusion_feature.detach().cpu().numpy()
        eeg_w_np = eeg_w.detach().cpu().numpy()
        np.save('./features/eeg_feature.npy', eeg_feature_np)
        np.save('./features/frmi_feature.npy', frmi_feature_np)
        np.save('./features/fusion_feature.npy', fusion_feature_np)
        np.save('./features/eeg_w.npy', eeg_w_np)
        np.save('./features/label', self.allLabel)
...
if __name__ == '__main__':
    log_path = './results'
    data_path = '/home/sy/data/multi_model/EEG/Sub_S1_single/Sub_S1_'
    data_path_r = '/home/sy/data/multi_model/fMRI/Sub_S1_single/Sub_S1_'
    epoch = 200
    batch_size = 32
    learning_rate = 0.001

    model = CogRepLKNet(in_feature=96, pretrained=False, drop_rate=0.2, save_feature=True)
    model_r = CogRepLKNet(in_feature=96, pretrained=False, drop_rate=0.2, save_feature=True)
    class_head = ClassificationHead(64 * 2, 2) # (128 * 2)
    AGCA_head = AdaptiveGatedCAHead(num_classes=2)
    ...
    r = list(range(5, 23)) + list(range(24, 35))
    for i in [5]:
        ...
        exp.get_feature(model, model_r, AGCA_head, i + 1, 'history_results/cogreplknet-default-wm-s1/model/')
        # exp.cal_para(model, model_r, rep=True)
        # acc, f1 = exp.train_and_val(model, model_r, AGCA_head, i + 1)
        # t = exp.test_normal_rep(model, model_r, AGCA_head, i + 1, 'history_results/cogreplknet-pretrained-wm-s2/model/', rep=False)
        # infer_time += t
        # print(acc, "   ", f1)
    # print("Inference time: %.2f s" % (infer_time / len(r)))
